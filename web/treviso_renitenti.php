...
function getFirstsParamsTreviso2() {
	...
	curl_setopt_array($curl, array(
	  CURLOPT_URL => "http://ricerchegenealogiche.archiviodistatotreviso.beniculturali.it/leva/renitenti.aspx",
...
function getPaginationTreviso2($page) {
...
function doSearchTreviso2($array, $cognome, $nome, $button, $page) {
...
function getOtherParamsTreviso2($page) {
...
function buildTableResultTreviso2($page) {
...
function finalResultTreviso2($cognome_param, $nome_param) {
	
	$first_params = getFirstsParamsTreviso2();
	
	$get_first_page = doSearchTreviso2($first_params, $cognome_param, $nome_param, true, '');
	
	$pages = getPaginationTreviso2($get_first_page); //11
	
	$other_params = getOtherParamsTreviso2($get_first_page);
	
	$count = 1;

	$response = array();
	
	$response = buildTableResultTreviso2($get_first_page);
	...
	while ($count != $pages):
		...
		$get_pages = doSearchTreviso2($other_params, $cognome_param, $nome_param, false, $count);
		
		$pages2 = getPaginationTreviso2($get_pages); //11
		
		$table_result = buildTableResultTreviso2($get_pages);
	...
	if(!empty($response)) {
		...
		$final_result['treviso2']['msg'] = 'OK!';
		$final_result['treviso2']['dados'] = $response;
		
	} else {
		
		//$final_result['treviso']['status'] = false;
		$final_result['treviso2']['msg'] = 'Nenhum Registro Encontrado!';
		$final_result['treviso2']['dados'] = null;

		
	}
	
	$final_result['treviso2']['total_registro'] = count($response);
